...
void InitCryptoFVector(CryptoFP self, string_t iv)
{
...
void ACRYPTO_DataInjection(CryptoFP self, string_t iv)
{
	int len = SUS_getStringLength(iv);
	self->matrix[0][0] = '1';
	self->matrix[0][1] = '1' ^ iv[0];
...
void ACRYPTO_Default_Initalization(CryptoFP self, string_t base, int len)
{
	for (int i = 0; i < self->w; i++) {
